...
import type { Course, Exercise, UserCourseProgress, AnswerResult } from '@/types/learn';
...
const completedIds = ref<number[]>(props.progress?.completed_exercise_ids ?? []);
const currentIndex = ref<number>((() => {
    if (!props.progress?.current_exercise_id) return 0;
    const idx = props.exercises.findIndex((e) => e.id === props.progress!.current_exercise_id);
    return idx >= 0 ? idx : 0;
})());
...
const progressPercent = computed(() =>
    props.exercises.length > 0 ? (completedIds.value.length / props.exercises.length) * 100 : 0
);

function handleAnswered(result: AnswerResult) {
    if (result.correct && currentExercise.value && !completedIds.value.includes(currentExercise.value.id)) {
        score.value += 1;
...
<template>
    ...
    <div class="flex min-h-screen flex-col bg-background">
        ...
        <header class="flex items-center justify-between border-b border-border px-4 py-3">
            <Link href="/courses" class="flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground">
                ← Back to courses
            </Link>
        ...
        <div class="border-b border-border bg-muted/30 px-4 py-4">
            <div class="mx-auto max-w-2xl">
                ...
                <h1 class="mt-1 text-xl font-bold text-foreground">{{ course.title }}</h1>
        ...
        <main class="flex flex-1 items-start justify-center px-4 py-8">
            <div class="w-full max-w-2xl">
                ...
                <div v-if="finished" class="space-y-6 text-center">
                    ...
                    <h2 class="text-2xl font-bold text-foreground">Course Complete!</h2>
                    <p class="text-muted-foreground">
                        You scored <span class="font-bold text-emerald-600">{{ score }}</span> out of
                        <span class="font-bold">{{ exercises.length }}</span> exercises correctly.
                    </p>
                    <div class="flex flex-col gap-3 sm:flex-row sm:justify-center">
                        ...
                        <button
                            ...
                            @click="currentIndex = 0; finished = false; completedIds = []; score = 0"
                        >
                            Restart course
                        </button>
                    </div>
                ...
                <div v-else class="rounded-xl border border-dashed border-border py-16 text-center">
                    <p class="text-muted-foreground">This course has no exercises yet.</p>
                </div>
            </div>
        </main>
    </div>
